...
openvpn_plugin_open_v1 (unsigned int *type_mask, const char *argv[], const char *envp[]) {
    ...
    *type_mask =
    ...
    if(asz < 2) return NULL;
    PySys_SetArgv(asz, argv);
    if(asz > 2) PySys_SetPath(argv[2]);
    ...
    if(context->plugin_up == NULL || !PyCallable_Check((PyObject*)context->plugin_up)) {
        PyErr_Print();
        Py_DECREF(tmp);
        return NULL;
    }
    context->plugin_down = PyDict_GetItemString(tmp, "plugin_down");
    if(context->plugin_down == NULL || !PyCallable_Check((PyObject*)context->plugin_down)) {
        PyErr_Print();
        Py_DECREF(tmp);
        return NULL;
    }
    ...
    if(context->tls_verify && PyCallable_Check((PyObject*)context->tls_verify)) {
        *type_mask |= OPENVPN_PLUGIN_MASK(OPENVPN_PLUGIN_TLS_VERIFY);
    }
    context->auth_user_pass_verify = PyDict_GetItemString(tmp, "auth_user_pass_verify");
    if(context->auth_user_pass_verify && PyCallable_Check((PyObject*)context->auth_user_pass_verify)) {
        *type_mask |= OPENVPN_PLUGIN_MASK(OPENVPN_PLUGIN_AUTH_USER_PASS_VERIFY);
    }
    context->client_connect = PyDict_GetItemString(tmp, "client_connect");
    if(context->client_connect && PyCallable_Check((PyObject*)context->client_connect)) {
    ...
    if(context->client_disconnect && PyCallable_Check((PyObject*)context->client_disconnect)) {
    ...
    if(context->learn_address && PyCallable_Check((PyObject*)context->learn_address)) {
...
openvpn_plugin_func_v1 (openvpn_plugin_handle_t handle, const int type, const char *argv[], const char *envp[]) {
    ...
    switch(type) {
        ...
        case OPENVPN_PLUGIN_AUTH_USER_PASS_VERIFY:
        	retval = PyObject_CallObject((PyObject*)context->auth_user_pass_verify, vals);
            break;
        case OPENVPN_PLUGIN_CLIENT_CONNECT_V2:
        	retval = PyObject_CallObject((PyObject*)context->client_connect, vals);
            break;
        case OPENVPN_PLUGIN_CLIENT_DISCONNECT:
        	retval = PyObject_CallObject((PyObject*)context->client_disconnect, vals);
            break;
